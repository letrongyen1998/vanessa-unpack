...
Функция СоздатьПустуюБазу(ПутьКФайлуКонфигурации = "", резИмяКаталогаБазы = "", ФайлСконвертирован = ЛОЖЬ) //Экспорт //СтруктураПараметров
	...
	ИначеЕсли НЕ ФайлСконвертирован Тогда // проверим на ошибку конвертации
		
		СписокСтрок = Новый Массив;
		СписокСтрок.Добавить("Структура конфигурации несовместима с текущей версией программы");	
		НадоКонвертировать = НЕ ПроверитьФайлНаОтсутствиеСтрок(АппЛог, СписокСтрок);
		Если НадоКонвертировать Тогда
		
			Команда = """"+мПараметры.ПутьКПлатформе83+"""" 
				+ " DESIGNER "+ПолучитьПодстрокуКомандыЗапуска1С_БазаДанных(ИмяКаталогаБазы)
				+ " /ConvertFiles"""+ПутьКФайлуКонфигурации+""""
				+ " /Lru"
				+ " /Out """+АппЛог+"""";
				
			ВывестиСообщение("Конвертация файла в новый формат");
			ВывестиСообщение(Команда);
			ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Команда);
			
			Возврат СоздатьПустуюБазу(ПутьКФайлуКонфигурации, резИмяКаталогаБазы, ИСТИНА); 
